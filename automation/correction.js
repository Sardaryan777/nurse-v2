...
export async function parseCorrectionInstructions(bodyText, batchSummary) {
  ...
  const resp = await anthropic.messages.create({
    ...
    system:
      ...
      "  painLevel (number 0-10), o2Sat (string), topic (string), nurseName (string),\n" +
      ...
      'Return ONLY compact JSON: {"regenerateAll":false,"changes":[{"noteIndex":0,"date":"MM/DD/YYYY","time":"07:00 PM","fields":{...}}],"ambiguous":["..."],"unmatched":["..."],"summary":"one line"}\n' +
...
async function rewriteNoteText(note, fields, poc) {
  const anthropic = client();
  const changeLines = [];
...
export async function applyCorrections(batch, parsed) {
  ...
  for (const change of parsed.changes) {
    ...
    const needsRewrite = f.painLevel != null || f.o2Sat || f.topic || f.discharge != null || f.instruction;
    if (needsRewrite) {
      try {
        note.intervention = await rewriteNoteText(note, f, note.poc || poc);
      } catch (err) {
